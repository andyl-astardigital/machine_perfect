...
This is not a convention. It's enforcement. `(set! x 5)` inside a guard or binding throws an error with the function name and guidance on where to put it.
...
This gives O(changed bindings) updates instead of O(all bindings). No Proxies, no compiler, no magic — just the fact that we control the evaluator.

The backend uses the same mechanism to know which views/projections need recalculating after a state transition.

## Error handling strategy

| Error type | Response |
|------------|----------|
| Malformed JSON in `mp-ctx` or `mp-store` | Warn, use empty object, continue |
| Unknown s-expression function | Warn, return null |
| Mutation in binding (`set!` in `mp-text`) | Throw with function name and guidance |
...
- **User functions are explicit** — `MachinePerfect.fn()` must be called to register. No implicit code execution.
...
An AI can generate a machine definition, then validate it structurally before any human sees it. This is not possible with arbitrary JavaScript.

## Computation formats vs data formats

Traditional systems exchange data: JSON payloads describing current state. The receiver needs prior knowledge of what to do with the data through API documentation, SDKs, or shared code.

machine_perfect exchanges computation: machine documents that describe behaviour. The receiver reads the document and knows:
- what states exist
- what transitions are legal from the current state
- what conditions must be met
- what will happen when a transition fires

This is the shift from REST ("here is a resource, here are the verbs") to machine-native ("here is a computation unit, here are its legal operations"). The machine document is self-describing, self-validating, and executable by any host that has the shared engine.

## Capability-based distribution (proposed — ADR-012)

Traditional distribution asks: "which SERVICE handles this?" Capability-based distribution asks: "which HOST can fulfil these EFFECTS?"

A transition declares what it needs:
```html
mp-where="(requires 'persist' 'notify')"
```
...
**Consequences:**
- Read one machine document and you see every state, transition, effect, and where it executes.
- Deployment is capability declaration: "I can persist and fulfil", not "I am the order service."
- Scaling is pool-level. Need more persistence throughput? Add instances to the persist pool. No API changes.
