...
## Rationale
- **Same evaluator** — the s-expression engine (`mp/engine.js`) runs in Node without modification. `require('./mp/engine.js')` gives you the same tokenizer, parser, evaluator, stdlib, and dependency tracker that runs in the browser.
...
- **Consistent with philosophy** — the framework's promise is "no build step, just run." `npx machine-perfect serve` is consistent. A Rust binary requires compilation.
...
- The upside outweighs these: one codebase, one language, one evaluator, zero sync overhead.
