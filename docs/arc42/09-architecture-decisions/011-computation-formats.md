...
## Decision
Machine definitions are treated as portable computation formats — documents that carry their own behaviour, rules, and legal operations. A machine document is simultaneously:
...
4. The definition runs in both browser (HTML) and server (SCXML) without modification
5. The same tooling (graph, lint, simulate, REPL) works for all applications

This eliminates the translation overhead between thinking and building. The mental model (state machines) maps directly to the implementation (machine definitions) maps directly to the runtime (shared engine).
