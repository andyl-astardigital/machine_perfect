...
## Rationale
- One syntax: the same language in `<mn-text>`, `<mn-on>`, `<mn-class>`, `<mn-receive>`, `<mn-guard>`, `<mn-action>`. Guards, actions, and bindings are s-expressions inside structural child elements. No context switching.
...
- Dependency trackable: all data access goes through `seval`, so the evaluator can instrument reads and writes without Proxies or a compiler.
...
- The expression language is closed. Adding new built-ins requires framework changes, not user code. `MachineNative.fn()` is the escape hatch.
