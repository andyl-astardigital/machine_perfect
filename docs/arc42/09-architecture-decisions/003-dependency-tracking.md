...
- 100% coverage: all data access flows through `seval` and symbol resolution, so we see every read. No opt-in, no wrapping, no decorators. Includes dynamic property access via `(get obj key)`.
